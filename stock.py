...
def load_json_config(): # 각 주식별로 어떤 설정을 했는지 config.json 파일에서 가져와서, 주식 객체 만들어줌.
    stocks = []

    for stock in json_stocks:
        with open("./stock_order_configs/"+stock.stock_code, "r") as json_file:
...
def save_json_config(stocks): # 각 주식에 수정 사항 있는데로 파일 저장. # db로 하는게 나을 듯..?
    for stock in stocks:
        stock_object = {
            ...
            "buy_last_time" : stock.buy_last_time,
            ...
            "sell_last_time" : stock.sell_last_time
        }
        with open("./stock_order_configs/"+stock.stock_code, "w") as json_file:
...
class Stock():
    ...
    def get_stock_amount(self):
        ...
        if(stock_df['종목코드'].str.contains(self.stock_code)[0]):
            ...
            return int(target_stock['보유수량']) # int()
        else:
            return 0
...
def trade_function(stocks): # 거래 결정 함수 (이걸 계속 돌리면 될 듯.)
    for stock in stocks:
        stock.buy_stock()
        time.sleep(1)
    time.sleep(2)
